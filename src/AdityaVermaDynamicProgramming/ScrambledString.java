...
public class ScrambledString {
    ...
    public boolean Dp(String s1, String s2) {
        ...
        int n = s1.length();
        for (int i = 1; i < n; i++) {
            boolean condition1 = Dp(s1.substring(0, i), s2.substring(n - i, n))
                    && Dp(s1.substring(i, n), s2.substring(0, n - i));
            if (condition1) {
                flag = true;
                break;
            }
            boolean condition2 = Dp(s1.substring(0, i), s2.substring(0, i))
                    && Dp(s1.substring(i, n), s2.substring(i, n));
            if (condition2) {
                flag = true;
                break;
            }
